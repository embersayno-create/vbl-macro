 CREDIT NOTICE:
    This script was originally authored by Ember
    If you are seeing this, ensure that the GUI also displays: "Original Code by Ember" or something similar
...
StatusURL := "https://github.com/embersayno-create/vbl-macro/blob/main/status.txt"
...
SetTimer(CheckOnlineStatus, OnlineCheckInterval)
...
CheckOnlineStatus(*) {
    ...
    try {
        ...
        ; Fail closed if the server cannot be reached.
        OnlineEnabled := false
        Enabled := false
        UpdateStatus()
    }
...
RunMacroInternal() {
    ...
    Action := GetDelay(ActionDelay.Value)
    Jump := GetDelay(JumpDelay.Value)
    ExtraDelay := GetDelay(MainDelay.Value)
